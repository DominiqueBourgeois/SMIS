function sample_figure_number=DisplaySample(smis_gui_parameters, CurrentFluo)
%
% PURPOSE:
%   Display sample
%
...
%   CurrentFluo: current fluorophore number
%
% OUTPUTS:
%   sample_figure_number: updated sample_figure_number
%
% MODIFICATION HISTORY:
%	D.Bourgeois, July 2025.
...
for k=1:n_fig
    if strcmp(figHandles(k).Name,'Virtual Sample')==1 % Our figure already exists
        create_figure=0;
        sample_figure_number=figHandles(k).Number;
    end
end
...
if create_figure==1
    ...
    sample_figure_number=sample_figure.Number;

    colormap(gray);
    set(sample_figure,'color','white');
    set(sample_figure,'Position',[10 scrsz(4)/10 w_xsize w_ysize])
    set(sample_figure,'color','white');
    set(sample_figure,'Name','Virtual Sample');
    set(sample_figure,'NumberTitle','off');
else
    figure(sample_figure_number);
    clf;
end
...
if ~isempty(CurrentFluo)
    im=smis_gui_parameters.in_images{CurrentFluo};
else
    im=[];
end

if ~isempty(im)
    CurrentFluorophoreName=smis_gui_parameters.Fluorophores(CurrentFluo).name;

    
    MinDisp=min(smis_gui_parameters.pattern_ids(CurrentFluo).sp_ids);
    MaxDisp=max(smis_gui_parameters.pattern_ids(CurrentFluo).sp_ids);

    if MinDisp<MaxDisp
        clims = [MinDisp, MaxDisp];
        imagesc(im,clims);
    else
        imagesc(im,Pattern);
    end
    axis image
    colormap('gray')
    str1=strrep(CurrentFluorophoreName,'_','\_');
    str2=strrep(smis_gui_parameters.in_images_names{CurrentFluo},'_','\_');
    MyTitle = {['Virtual sample for fluorophore: ', str1],['(',str2,')']};
    title(MyTitle)
    ...
    clf;
    MyDlg=warndlg(['No virtual sample loaded for fluorophore: ', CurrentFluorophoreName,' !'],'warning');
    waitfor(MyDlg)
end
...
% sample_figure_number=smis_gui_parameters.sample_figure_number;
% 
