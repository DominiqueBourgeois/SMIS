...
function smis_ok=smis_vsn2_3_main(app, smis_par)
...
%SMIS2.3: Added hop diffusion, flexible assignment to fluorescent state to
...
in_images= smis_par.in_images;
in_image_size= smis_par.image_size;
in_image_n_sp=[smis_par.pattern_ids.n_sp]; % # of subpatterns
...
for k=1:n_lasers
    ...
    lasers(k).mask_pattern=smis_par.lasers(k).mask_pattern;
    lasers(k).beam_profile=smis_par.lasers(k).beam_profile;
end
...
for k=1:n_fluorophores
    ...
    if smis_par.Fluorophores(k).Motion.Hop_Diffusion==1
        sm_par(k).Hop_Diffusion=1;
        sm_par(k).Hop_Probability =smis_par.Fluorophores(k).Motion.Hop_Probability; % [um/s] Array of velocities for first fluorophore (directed diffusion)
    else
        sm_par(k).Hop_Diffusion=0;
        sm_par(k).Hop_Probability =zeros(1,numel(smis_par.Fluorophores(k).Motion.D));
    end
...
textured_bg_pattern=smis_par.BG.textured_bg_pattern;
...
if add_textured_bg==1
    if isempty(textured_bg_pattern)
...
for i=1:n_lasers
    if isscalar(lasers(i).sequence); lasers(i).sequence=lasers(i).sequence*ones(1,n_images); end
end

for i=1:n_lasers
    if isempty(lasers(i).beam_profile)
        SMISMessage=['Laser beam profile is undefined ! Check: ''', lasers(i).name, ''' Aborting !'];
        disp(SMISMessage);
        warndlg(SMISMessage,'Warning')
        smis_ok=0; return
    end

    if size(lasers(i).beam_profile,1)~=smis_par.image_size.n || size(lasers(i).beam_profile,2)~=smis_par.image_size.m
        SMISMessage=['Laser beam profile dimension does not match detector dimension ! Check: ''', lasers(i).name, ''' Aborting !'];
        disp(SMISMessage);
        warndlg(SMISMessage,'Warning')
        smis_ok=0; return
    end
...
% Virtual samples
% Check if virtual samples already loaded in smis_par
unloaded_vs=cellfun(@isempty,in_images);
if nnz(unloaded_vs)>0 % There is at least a non-loaded pattern, so load them all again
    for i=1:n_fluorophores
        if isempty(in_images(i)) % Only if virtual images not already contained in smis_par
            ok=check_input(char(in_images_dir(i)),char(in_images_names(i)));
            if ok==0
                MyFile=fullfile(char(in_images_dir(i)),char(in_images_names(i)));
                SMISMessage=['Virtual sample file: ', MyFile, ' does not exist !'];
                disp(SMISMessage);
                warndlg(SMISMessage,'Warning')
                smis_ok=0; return
            end
...
for i=1:n_lasers
    if lasers(i).frap_mode==1 && isempty(lasers(i).mask_pattern)
...
for i=1:n_fluorophores
    ...
    if sm_par(i).pH_sensitivity==1
        for k=1:sm_par(i).n_fluorescent_states
            ...
            associated_dark_state_id=find(sm_par(i).state_ids==sm_par(i).fluorescent_states(k) & ((1:sm_par(i).n_states)' ~= sm_par(i).fluorescent_states(k)));
            if ~isempty(associated_dark_state_id)
                sm_par(i).associated_dark_states(k)=associated_dark_state_id;
                ...
                sm_par(i).associated_dark_states(k)=nan;
                sm_par(i).fluorescent_fraction(k)=1; % Do not share anything with a dark state
            end
        end
    end
...
im_par.bg=struct(...
    ...
    'textured_bg_pattern', textured_bg_pattern, ...
...
%% Read virtual samples
% Check if virtual samples already loaded in smis_par
unloaded_vs=cellfun(@isempty,in_images);
if nnz(unloaded_vs)>0 % There is at least a non-loaded pattern, so load them all again
    [a_h_all,im_par.n,im_par.m,im_par.nz,im_par.n_sp] = read_patterns(in_images_dir, in_images_names, im_par, simul_3D);
    if isempty(a_h_all)
...
else % all virtual samples already available in smis_par
    % code valid for both 2D and 3D
    a_h_all=zeros([n_fluorophores,size(in_images{1})]);
    for i=1:n_fluorophores
        a_h_all(i,:,:,:)=smis_par.in_images{i}; 
    end
    im_par.n=in_image_size.n;  % Size of detected (binned) image
    im_par.m=in_image_size.m;
    im_par.nz=in_image_size.nz;
    im_par.n_sp=in_image_n_sp;
end
...
if distribute_in_clusters==0
    for i=1:n_fluorophores
        ...
        if im_par.n_sp(i)~=sm_par(i).n_sp
...
if im_par.bg.add_textured_bg==1
    % read the pattern file if necessary
    if isempty(im_par.bg.textured_bg_pattern)
...
%Display image size
disp(['Imaged sample size is [um, um]: ',num2str(0.001*raster*[im_par.n,im_par.m])])
if simul_3D==1
    disp(['Sample thickness is [um]: ',num2str(0.001*raster*im_par.nz)])
end
...
for i=1:n_lasers
    if lasers(i).frap_mode == 1
        if ~isempty(lasers(i).mask_pattern)
            frap_zone=lasers(i).mask_pattern;
        else
            frap_zone=imread([lasers(i).mask_dir,lasers(i).mask_file]);
            frap_zone=round(frap_zone/max(frap_zone(:))); % Normalize to 1 and allow only 0 and 1
        end
    ...
    lasers(i).mask_pattern=[]; %No need to keep this in memory
end
