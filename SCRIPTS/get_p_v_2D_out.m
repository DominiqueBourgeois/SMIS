function  [P_out, V_out]=get_p_v_2D_out(P_in, V_in, S, v, dt, raster, binning, margin_factor, w, sz)

% PURPOSE:
%   Select velocity vector which is closest to the initial direction, and that allows for the molecule to
%   stay within the pattern at its current speed. 2D case
%
% INPUTS:
...
int=intersect(sub2ind(sz,round(x2),round(y2)),w); % the indices of the circle on the current pattern

if ~isempty(int) % This should be the case unless the pattern is too small locally
    %The possible reached pixels are
    [tp_x, tp_y]=ind2sub(sz,int); % x,y, z coordinates corresponding to ind_cp_s
    
    %The possible new vectors on the unit circle are
    V=[tp_x'-round(P_in(1)); tp_y'-round(P_in(2))];
    U=V./vecnorm(V);
    ...
    if any([P_out(1)<1 P_out(2)<1]) || any([P_out(1)>sz(1) P_out(2)>sz(2)]) % check if molecule is inside the FOV
    ...
    V_out=[0,0]; % Simply reverse the speed
    P_out=P_in; % Simply do not move the molecule
end
