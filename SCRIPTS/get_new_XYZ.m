function [x_h_d, y_h_d, z_h_d, sm] = get_new_XYZ(x_h, y_h, z_h, sm, D_ras, dt, S, sm_par, im_par)

% PURPOSE:
% Get new position for a molecule initially at x,y diffusing in 3D for time dt with diffusion coeff D
%
% INPUTS:
...
if sm_par.V(c_ds)>0
    [p_h_out,v_h_out,p_h_out_np]=update_directed_motion_3D([x_h,y_h,z_h],[sm.v_x, sm.v_y, sm.v_z], dt, c_ds, n_ds, sm_par,im_par);
    ...
    if isempty(p_h_out_np) % Normal case, no change of pattern possible
        ...
        % eventually update current diffusion state and velocity
        ...
        %             sm.v_z=v_h_out(3)/norm(v_h_out)*1e+3*sm_par.V(n_ds)/im_par.raster*im_par.binning;
        %         end
...
if D_ras_XY(c_ds)==0 && D_ras_Z(c_ds)==0
    ...
    if n_ds~=c_ds && sm_par.D_confined(n_ds)==sm_par.D_confined(c_ds) % Case where there is
        ...
        if sm_par.V(c_ds)>0 % eventually update current velocity Changed 17/01/21
            v_h=[sm.v_x, sm.v_y, sm.v_z]; % This is the current velocity vector
            ...
            sm.v_z=v_h(3)/norm(v_h)*1e+3*sm_par.V(n_ds)/im_par.raster*im_par.binning;
