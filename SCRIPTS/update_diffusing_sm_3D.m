function  sm = update_diffusing_sm_3D(sm, im_par, sm_par, S_DS)
...
S=[im_par.binning*im_par.n, im_par.binning*im_par.m, im_par.binning*im_par.nz]; % size of high-resolution image
...
D_ras_xy=1e+06*D/raster^2; % Ex: if D = 0.01 um2/s = 100 nm^2/s = (10 nm x 10 nm)/s, and pixel size = 100 nm, in pixel D = 0.01 pix^2/s
D_ras_z=1e+06*D/raster^2; % Ex: if D = 0.01 um2/s = D = 100 nm^2/s = (10 nm x 10 nm)/s, and pixel size = 100 nm, in pixel D = 0.01 pix^2/s
% D_ras_z may differ from D_ras_xy in case raster size in z would be different, not yet implemented !!');
D_ras0=[D_ras_xy;D_ras_z];
...
z_h=[sm.z]; % z current position of the molecules
% z_h_d=z_h; % replicate for moved z
...
parfor (k=1:numel(sm), im_par.parforArg) % Go for all activated molecules
    ...
    if sm(k).activated==current_frame
        ...
        sm(k).z_track=[z_h(k),current_frame];
    end
    ...
    if use_diffuse_psf==1
        ...
        if dt_addtime > 0
            if max_dt>=dt_addtime % In that case no need to subdivide
                ...
                [sm(k).n_diff_state, sm(k).n_sp]=get_pattern_transition([x_h(k),y_h(k),z_h(k)], ...
                ...
                [x_h_d_tmp, y_h_d_tmp, z_h_d_tmp, sm(k)]=get_new_XYZ(x_h(k), y_h(k), z_h(k), sm(k), D_ras, dt_addtime, S, sm_par, im_par);
                ...
                sub_xyz=zeros(n_substeps+1,3); % define the intermediate positions sub_x, sub_y and sub_z
                sub_xyz(1,:)=[x_h(k), y_h(k), z_h(k)]; % starting position
                ...
                for k2=1:n_substeps
                    % Look for a potential change in diffusion state
                    [sm(k).n_diff_state, sm(k).n_sp]=get_pattern_transition([sub_xyz(k2,1),sub_xyz(k2,2),sub_xyz(k2,3)], ...
                        V_ras(c_diff_state), D_ras(c_diff_state), c_diff_state,K,C,dt_subaddtime,sm_par,im_par); % Get new diffusion coefficient                   
                    
                    % Update position and check if a potential change in diffusion state is realized or not
                    [sub_xyz(k2+1,1),sub_xyz(k2+1,2),sub_xyz(k2+1,3),sm(k)]=get_new_XYZ(sub_xyz(k2,1),sub_xyz(k2,2),sub_xyz(k2,3), sm(k), ...
                ...
                x_h_d_tmp=sub_xyz(end,1); % ending position
                y_h_d_tmp=sub_xyz(end,2);
                z_h_d_tmp=sub_xyz(end,3);
            end
        ...
        % Look for a potential change in diffusion state
        [sm(k).n_diff_state, sm(k).n_sp]=get_pattern_transition([x_h_d_tmp,y_h_d_tmp,z_h_d_tmp], ...
            V_ras(N_DS), D_ras(N_DS), N_DS,K,C,dt_frametime,sm_par,im_par); 
        
        % Update position and check if a potential change in diffusion state is realized or not
        [x_h_d_tmp2, y_h_d_tmp2, z_h_d_tmp2,sm_tmp2]=get_new_XYZ(x_h_d_tmp, y_h_d_tmp, z_h_d_tmp, ...
        ...
        if diffused_distance>diffuse_psf_radius || max_dt<dt_frametime 
            ...
            sub_xyz=zeros(n_substeps+1,3); % define the intermediate positions sub_x, sub_y and sub_z
            sub_xyz(1,:)=[x_h_d_tmp, y_h_d_tmp, z_h_d_tmp]; % starting position
            ...
            x_h_d=sub_xyz(end,1); % ending position
            y_h_d=sub_xyz(end,2);
            z_h_d=sub_xyz(end,3);
        ...
        % Look for a change in diffusion state over addtime plus frametime
        [sm(k).n_diff_state, sm(k).n_sp]=get_pattern_transition([x_h(k),y_h(k),z_h(k)], V_ras(S_DS), D_ras(S_DS), S_DS,K,C,dt,sm_par,im_par); % Get new diffusion coefficient
        
        % Update position and check if a potential change in diffusion state is realized or not
        [x_h_d, y_h_d, z_h_d, sm(k)]=get_new_XYZ(x_h(k), y_h(k), z_h(k), sm(k), D_ras, dt, S, sm_par, im_par);
    ...
    if use_diffuse_psf==1
        if diffused_distance>diffuse_psf_radius
            sm(k).sub_x=sub_xyz(:,1);
            sm(k).sub_y=sub_xyz(:,2);
            sm(k).sub_z=sub_xyz(:,3);
        else
            sm(k).sub_x=[];
            sm(k).sub_y=[];
            sm(k).sub_z=[];
    ...
    sm(k).z_track=vertcat(sm(k).z_track, [z_h_d,current_frame+1]);
end
