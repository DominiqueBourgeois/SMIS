...
%
% INPUTS:
% 	p_h: the xyz-coordinate on high-resolution image [raster]
%   v_h: the xyz-coordinate of the initial velocity vector on high-resolution image [raster]
...
%	p_h_out_np: [pixels] the updated xyz-coordinate on high-resolution image if molecule is allowed to jump to new pattern
...
ind = sub2ind(sz,round(x),round(y),round(z)); % the index associated to the circle
...
if show==1 % Set up image if we want to see it
    I=zeros(sz);
    I(c_w_pattern)=10;
    I(round(p_h(1)), round(p_h(2)), round(p_h(3)))=50;
    I(ind)=20;
...
if ~isempty(ind_cp)  
    ...
    [u_m, I]=get_directions_on_3D_pattern(ind_cp, round(p_h(1)), round(p_h(2)), round(p_h(3)), sz, show, I, D);
    ...
    if norm(u_m(:,1))<1e-3 % Do not use a limit of zero, the dispersion might not be exactly 0 for a perfect circle
        ...
        if size(u_m,2)==1 % This situation might happen when a molecule bump into a new pattern. In that case, we will choose 
            ...
            [x_f, y_f, z_f]=ind2sub(sz,ind_f); % x,y, z coordinates corresponding to tp_f
            
            % Define the chosen unit vector u_f
            v=[x_f'-round(p_h(1)); y_f'-round(p_h(2)); z_f'-round(p_h(3))];
            u_f=v./vecnorm(v); 
    ...
    if ~(any([p_h_out(1)<1 p_h_out(2)<1 p_h_out(3)<1]) || any([p_h_out(1)>sz(1) p_h_out(2)>sz(2) p_h_out(3)>sz(3)])) % check if molecule is inside the FOV
        
        check_ind = sub2ind(sz,round(p_h_out(1)),round(p_h_out(2)),round(p_h_out(3))); % the index associated to the new position
        ...
        if check_inside==0 % In that case the molecule does not end up with in the pattern and 
            ...
            [p_h_out, v_h_out]=get_p_v_3D_out(p_h, v_h, sm_par.V_circle(c_ds), sm_par.V(c_ds), dt, im_par.raster, im_par.binning, sm_par.margin_factor, c_w_pattern, sz);
        end
...
if show==1
    figure(1)
    clf
    line([p_h(2),p_h_out(2)],[p_h(1),p_h_out(1)],[p_h(3),p_h_out(3)],'Color','red','LineWidth',3);
    hold on
    vol3d('cdata', I);
    alphamap('default')
    axis image
...
if n_ds~=c_ds  && any(cp_id~=np_id) % Corrected 17/01/21
    ...
    while ~on_new_pattern && (k>0)
        p_h_out_np=p_h+k/n_pos*dt*v_h; % Possible position on the new pattern
            if ~(any([p_h_out_np(1)<1 p_h_out_np(2)<1 p_h_out_np(3)<1]) || any([p_h_out_np(1)>sz(1) p_h_out_np(2)>sz(2) p_h_out_np(3)>sz(3)])) % check if molecule is inside the FOV
                ind_np = sub2ind(sz,round(p_h_out_np(1)),round(p_h_out_np(2)),round(p_h_out_np(3))); % the index associated to the new position
