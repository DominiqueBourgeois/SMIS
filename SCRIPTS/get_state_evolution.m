function sm = get_state_evolution(sm, lasers, sm_par, im_par, w_events, starting_states, ending_states, during_frametime)
%
% PURPOSE:
...
if im_par.during_frametime==1
    ...
    if ~isnan(sm.sampling_rate(1))
        sampling_rate=sm.sampling_rate(1);
    else
        sampling_rate=sm_par.start_sampling_rate(1);
    end
else
    T=1e-3*im_par.addtime;
    if ~isnan(sm.sampling_rate(2))
        sampling_rate=sm.sampling_rate(2);
    else
...
% state
[trans_p, allowed_n_trans, allowed_trans_indices]=get_allowed_transitions(trans_p, sm, sm_par);
...
sm_trace=[0;sm.state]; % define (local) trace at start
...
if ~isempty(Trace)
    ...
    for i=1:size(Trace,2)
        ...
        if sm.state_id == sm_par.state_ids(S)
            ...
            if Trace(1,i)*T/N_trials ~= sm_trace(1,end)
                ...
                sm.state = ending_states(2,ending_states(1,:)==t); % update sm state
                sm.state_id = sm_par.state_ids(sm.state); % update sm state_id
                sm_trace = horzcat(sm_trace,[Trace(1,i)*T/N_trials;sm.state]); % final state
            end
        end
        % Stop in case of photobleaching
        if min(abs(sm.state-[sm_par.bleached_states]))==0
            sm.bleached=im_par.current_frame;
            sm.blinked=0;
            break;
        end
    end
    
    sm_trace = horzcat(sm_trace,[T ;sm.state]); % close up the trace

    if (sm.activated==0 && sm.state>=sm_par.converted_state); sm.activated=im_par.current_frame; end % update activation state if photoconversion occurred
    
else % no change in photophysical state
    sm_trace = horzcat(sm_trace,[T ;sm.state]); % simply close up trace
end

sm.state_trace=sm_trace;  % updated sm state trace
...
sm.fluo_trace=fluo_trace;  % updated fluorescent state

%update blinking status at the end of the trace
if min(abs(sm_par.state_ids(sm_trace(2,end))-[sm_par.fluorescent_states]))~=0 && sm.bleached==0
    sm.blinked=1;
else
    sm.blinked=0;
end
...
%update total traces
sm.tot_state_trace=update_tot_trace(sm, im_par);

%update Ns and sampling rate
if im_par.during_frametime==1
    sm.Ns(end,:)=Ns;
    sm.sampling_rate(1)=sampling_rate;
else
    sm.Ns(1,:)=Ns;
    sm.sampling_rate(2)=sampling_rate;
end
...
if plot_traces==1
    ...
    disp(['Single molecule id:' , num2str(sm.id)]);
    disp(['Photophysical state at end of trace:' , num2str(sm.state)]);
    drawnow
    input('Ok ? ','s');
end
