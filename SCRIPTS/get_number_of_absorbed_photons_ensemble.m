function N=get_number_of_absorbed_photons_ensemble(sp, lasers, sm_par, im_par)

% PURPOSE:
%   Get the number of absorbed photon per sampling time from each photoactive state according
%   to sub population position, extinction coefficient, laser power
%   density 
%
% INPUTS:
...
%
% OUTPUTS:
%   N: number of photons absorbed per second 
%
...
%                 if sp.state<sm_par.converted_state || (sp.state>=sm_par.converted_state && fluo_state_id >= sm_par.converted_state)
...
%                 end
            ...
            for j=1:sm_par.n_photoactive_dark_states
                ...
                % Only scan for possible states if molecule already photoconverted
%                 if sp.state<sm_par.converted_state || (sp.state>=sm_par.converted_state && dark_state_id >= sm_par.converted_state)
                    %The cross section is given by
