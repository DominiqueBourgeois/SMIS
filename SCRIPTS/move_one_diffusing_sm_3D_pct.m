...
z_h=sm{z_idx}; % z current position of the molecule
...
if sm{activated_idx}==current_frame
    ...
    sm{z_track_idx}=[z_h,current_frame];
end
...
if use_diffuse_psf==1
    ...
    if dt_addtime > 0
        if max_dt>=dt_addtime % In that case no need to subdivide
            ...
            sub_xyz(1,:)=[x_h, y_h, z_h]; % starting position
...
if use_diffuse_psf==1
    if diffused_distance>diffuse_psf_radius
        sm{sub_x_idx}=sub_xyz(:,1);
        sm{sub_y_idx}=sub_xyz(:,2);
        sm{sub_z_idx}=sub_xyz(:,3);
    else
        sm{sub_x_idx}=[];
        sm{sub_y_idx}=[];
        sm{sub_z_idx}=[];
...
sm{z_track_idx}=vertcat(sm{z_track_idx}, [z_h_d,current_frame+1]);
