function sm_td=get_new_XYZ_td_pct(sm, sm_td, im_par, pair_id)
...
sm_td{x_idx} = sm{x_idx} + d*cos(theta)*cos(phi);
sm_td{y_idx} = sm{y_idx} + d*sin(theta)*cos(phi);
sm_td{z_idx} = sm{z_idx} + d*sin(phi);
% do the same for intermediate xy if necessary
if im_par.use_diffuse_psf==1
    sm_td{sub_x_idx} = sm{sub_x_idx} + d*cos(theta)*cos(phi);
    sm_td{sub_y_idx} = sm{sub_y_idx} + d*sin(theta)*cos(phi);
    sm_td{sub_z_idx} = sm{sub_z_idx} + d*sin(phi);
end