function [u_mean, im]=get_directions_on_2D_pattern(ind, x0, y0, s, show, im, D)

% PURPOSE:
% Get unit vectors from a point x0, y0 to a series of indices in a 2D
...
if numel(ind)>1 % Only do it if necessary
    ...
    s1=s(1); % The size of a line along the second dimension of the 2D image
    ...
    while size(gt2,1)>1 && ~isempty(w1)
        ...
        while ~isempty(w0) && ~isempty(w1)
            ...
            prox_2D_coord=[w-1,w+1,w-1+s1,w+s1,w+1+s1,w-1-s1,w-s1,w+1-s1];
            
            w2=ismember(gt2(w1,1),prox_2D_coord);
    ...
    u_mean=zeros(2,gn); % The average direction and dispersion for each group
    
    for k=1:gn
        [x, y]=ind2sub(s,gt(gt(:,2)==k,1)); % x,y coordinates corresponding to ind
        
        %Get the Unit vectors from x,y
        v=[x'-round(x0); y'-round(y0)];
        u=v./vecnorm(v);
...
if show==1 % Set up image if we want to see it
    ...
    I_loc=I(max([1,xc-D]):min([xc+D,s(1)]),max([1,yc-D]):min([yc+D,s(2)]));
    figure(1)
    clf
    imagesc(I_loc)
    axis image
    set(gcf,'color','white')
    title('2D View of the selected regions')
end
