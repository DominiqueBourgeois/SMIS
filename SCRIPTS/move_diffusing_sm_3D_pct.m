...
for i=1:n_fluorophores
    ...
    sub_z_idx=w_idx(9);
    id_idx=w_idx(15);
    diff_state_idx=w_idx(18);
    matched_idx=w_idx(21);
    ...
    if ~sm_par(i).is_acceptor
        ...
        if display_par.show_diff_image==1
            scatter3(im_par.m*im_par.binning-[sms(i).sm_cell{y_idx,sm_par(i).w_act}],...
                im_par.n*im_par.binning-[sms(i).sm_cell{x_idx,sm_par(i).w_act}],...
                im_par.nz*im_par.binning-[sms(i).sm_cell{z_idx,sm_par(i).w_act}],...
                display_par.diffusion_colors(i).c(sm_par(i).w_act));
        ...
        if im_par.fluorophore_pairing_on==1
            ...
            if ~isempty(w_td_act)
                ...
                for k=1:numel(w_td_act)
                    ...
                    if im_par.use_diffuse_psf==1 % Initialize field sub_x sub_y
                        ...
                        sms(td_id).sm_cell{sub_z_idx,w_td_act(k)} = [];
                    end
                    idx=[x_idx,y_idx,z_idx,sub_x_idx,sub_y_idx,sub_z_idx];
                    sms(td_id).sm_cell(idx,w_td_act(k))=get_new_XYZ_td_pct(sms(i).sm_cell(idx,[sms(i).sm_cell{id_idx,:}]==m), sms(td_id).sm_cell(idx,w_td_act(k)), im_par, pair_id);
                    ...
                    sms(td_id).sm_cell{z_track_idx,w_td_act(k)}=vertcat(sms(td_id).sm_cell{z_track_idx,w_td_act(k)}, [sms(td_id).sm_cell{z_idx,w_td_act(k)},im_par.current_frame]);
                end
            end
            ...
            if display_par.show_diff_image==1
                scatter3(im_par.m*im_par.binning-[sms(td_id).sm_cell{y_idx,sm_par(td_id).w_act}],...
                    im_par.n*im_par.binning-[sms(td_id).sm_cell{x_idx,sm_par(td_id).w_act}],...
                    im_par.nz*im_par.binning-[sms(td_id).sm_cell{z_idx,sm_par(td_id).w_act}],...
                    display_par.diffusion_colors(td_id).c(sm_par(td_id).w_act));
                if ~display_par.show_experiment ; pause(0.1); end % seems to be necessary for proper refresh
            end           
        end
    end
