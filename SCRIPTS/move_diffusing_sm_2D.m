function  sms = move_diffusing_sm_2D(n_fluorophores, sms, im_par, sm_par, display_par)
...
for i=1:n_fluorophores
    ...
    if ~sm_par(i).is_acceptor
        ...
        for j=1:N
            ...
            if ~isempty(w_D_act)
                sms(i).sm(w_D_act) = update_diffusing_sm_2D(sms(i).sm(w_D_act), im_par, sm_par(i), j);
            end
        end
                      
        if display_par.show_diff_image==1
            scatter(im_par.m*im_par.binning-[sms(i).sm(sm_par(i).w_act).y],...
                im_par.n*im_par.binning-[sms(i).sm(sm_par(i).w_act).x],1,display_par.diffusion_colors(i).c(sm_par(i).w_act));
        ...
        if im_par.fluorophore_pairing_on==1
            ...
            [~,w_td_act]=intersect([sms(td_id).sm.id],[sms(i).sm(w_act).matched]); % this gets the index of fluorophores of type "i" that are actually matched to an activated tandem
            if ~isempty(w_td_act)
                ...
                for k=1:numel(w_td_act)
                    ...
                    sms(td_id).sm(w_td_act(k))=get_new_XY_td(sms(i).sm([sms(i).sm.id]==m), sms(td_id).sm(w_td_act(k)), im_par, pair_id);
            ...
            if ~isempty(w_unmatched)
                ...
                for j=1:N
                    ...
                    if ~isempty(w_D_act)
                        sms(td_id).sm(w_D_act) = update_diffusing_sm_2D(sms(td_id).sm(w_D_act), im_par, sm_par(td_id), j);
                    end
                end
            end
                                    
            if display_par.show_diff_image==1
                scatter(im_par.m*im_par.binning-[sms(td_id).sm(sm_par(td_id).w_act).y],...
                    im_par.n*im_par.binning-[sms(td_id).sm(sm_par(td_id).w_act).x],1,display_par.diffusion_colors(td_id).c(sm_par(td_id).w_act));
                if ~display_par.show_experiment ; pause(0.1); end % seems to be necessary for proper refresh
            end           
        end
    end
