function  sm_cell = update_diffusing_sm_3D_pct(sm_cell, im_par, sm_pattern_indices, sm_par, S_DS)
...
%	D.Bourgeois, February 2023, introduce sm_pattern_indices, now disconnected from sm_par. Parallel computing not run anymore in 3D due to the large size of the broadcast variables.
...
% Due to potentially huge size of broadcast variable sm_pattern_indices, do
% not run pct here.
for k=1:n_mol
    sm(:,k)=move_one_diffusing_sm_3D_pct(sm(:,k), sm_par, sm_pattern_indices, im_par, S_DS);
end
