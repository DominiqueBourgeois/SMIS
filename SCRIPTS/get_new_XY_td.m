function sm_td=get_new_XY_td(sm, sm_td, im_par, pair_id)
...
sm_td.x = sm.x + d.*cos(theta);
sm_td.y = sm.y + d.*cos(theta);
% do the same for intermediate xy if necessary
if im_par.use_diffuse_psf==1
    sm_td.sub_x = sm.sub_x + d.*cos(theta);
    sm_td.sub_y = sm.sub_y + d.*cos(theta);
end
end
