function [x,y,z,sp,a_sm,sm_par,ok] = place_single_molecules_simple(a,im_par,sm_par)
% NAME:
...
%
% CALLING SEQUENCE:
%	[x,y,z,sp,a_sm] = place_single_molecules(a,im_par,sm_par)
%
% INPUTS:
%	a: the high-resolution image onto place the SMs
...
%define x, y, and z
x=zeros(sm_par.n_mol,1);
y=zeros(sm_par.n_mol,1);
z=zeros(sm_par.n_mol,1);
sp=zeros(sm_par.n_mol,1);
...
for k=1:n_sp
    n_mol=fix(sm_par.n_mol*fr(k)); % # of molecules to be placed on that subpattern
    ...
    if n_mol>0
        ...
        if numel(size(a))==2 % 2D image
            ...
            if fix(im_par.raster/sm_par.near_dist/im_par.binning)<1
                disp(['Fluorophore: ',sm_par.fluorophore_name]);
                disp('Decrease binning ! ');
                errordlg(['Maximum nearest distance between fluorophores is: ', num2str(im_par.raster/im_par.binning)],'SMIS Error');
                uiwait
                ok=0;
                return
            end
            ...
            if n_to_label==0
                ...
                disp(['Possible pattern ids for this pattern from: ', num2str(min(a(:))) ,' to: ',num2str(max(a(:)))]);
                errordlg(['This pattern id does not exist for this pattern: ', num2str(n_sp_id(k))],'SMIS Error');
                uiwait
                ok=0;
                return
            end
            ...
            if n_to_label<n_mol
                ...
                SMISMessage2=['Number of possibly labeled pixels for subpattern ', num2str(n_sp_id(k)) ,' : ',num2str(n_to_label)];
                SMISMessage=[SMISMessage1 newline SMISMessage2];
                disp(SMISMessage);
                
                SMISMessage1='Number of molecules to label is too high for allowed density !';
                SMISMessage2='Reduce # of molecules or reduce min distance between fluorophores ';
                SMISMessage=[SMISMessage1 newline SMISMessage2];
            ...
            if n_to_label>max_n_to_label
                SMISMessage1=['Fluorophore: ',sm_par.fluorophore_name];
                SMISMessage2=['Number of possibly labeled pixels for subpattern ', num2str(n_sp_id(k)), ' is too high for memory: ',num2str(n_to_label)];
                SMISMessage3='Increase min distance between fluorophores  , or decrease ''binning'' or ''raster'' parameters';
                SMISMessage=[SMISMessage1 newline SMISMessage2 newline SMISMessage3];
                disp(SMISMessage);
                warndlg(SMISMessage,'Warning')
                uiwait
                ok=0;
            ...
            if max(sorted_i_a) > length(w_a)
                ...
                disp(['Fluorophore: ',sm_par.fluorophore_name]);
                errordlg('Retry or reduce distance between fluorophores !','SMIS Error');
                uiwait
                ok=0;
                return
            end
            ...
            sp(mol_id+1:mol_id+n_mol)=n_sp_id(k); % value of the subpattern
