function [donor, donor_par, acceptor, acceptor_par] = process_fret_photophysics(donor, donor_par, acceptor, acceptor_par, lasers, im_par)
...
%
% INPUTS:
%   donor: the donor single molecules (with coordinates on high-resolution image in raster units)
%	donor_par: the sm parameters of the donor
%   acceptor: the acceptor single molecules (with coordinates on high-resolution image in raster units)
...
if acceptor_par.n_mol_eff>donor_par.n_mol_eff
    ...
    unpaired_indices_A=find([acceptor.sm.matched]==0); % Indices of acceptors that are unpaired
...
elseif donor_par.n_mol_eff>acceptor_par.n_mol_eff
    ...
    unpaired_indices_D=find([donor.sm.matched]==0); % Indices of donors that are unpaired
    paired_indices_D=find([donor.sm.matched]>0); % Indices of donors that are paired
else
    unpaired_indices_A=[]; % All acceptors are effectively paired
    unpaired_indices_D=[]; % All donors are effectively paired
...
acceptor_ids=[acceptor.sm.id];
for k=1:n_pairs
    paired_indice=find(acceptor_ids==donor.sm(paired_indices_D(k)).matched,1);
...
donor_sm=donor.sm(paired_indices_D);
if ~isempty(unpaired_indices_D)
    unpaired_donor_sm=donor.sm(unpaired_indices_D);
end

acceptor_sm=acceptor.sm(paired_indices_A);
if ~isempty(unpaired_indices_A)
    unpaired_acceptor_sm=acceptor.sm(unpaired_indices_A);
end
...
parfor (k=1:n_pairs, im_par.parforArg)
    ...
    if addtime>0
        ...
        during_frametime=0;
        %         [donor_sm(paired_indices_D(k)), ~, acceptor_sm(acceptor_ids==donor_sm(paired_indices_D(k)).matched), ~]=...
        %             get_fret_state_evolution(donor_sm(paired_indices_D(k)),donor_par, acceptor_sm(acceptor_ids==....
        %             donor_sm(paired_indices_D(k)).matched), acceptor_par, lasers, im_par, ...
        ...
        [donor_sm(k), acceptor_sm(k)]=...
            get_fret_state_evolution(donor_sm(k),donor_par, acceptor_sm(k), acceptor_par, lasers, im_par, ...
            w_events_D, starting_states_D, ending_states_D, w_events_A, starting_states_A, ending_states_A, ...
            state_link_D, state_link_A, fluo_states_D, fluo_states_A, R0_link_D, R0_link_A,during_frametime);
    ...
    during_frametime=1;
    %     [donor_sm(paired_indices_D(k)), donor_par, acceptor_sm(acceptor_ids==donor_sm(paired_indices_D(k)).matched), acceptor_par]=...
    %         get_fret_state_evolution(donor_sm(paired_indices_D(k)),donor_par, acceptor_sm(acceptor_ids==....
    %         donor_sm(paired_indices_D(k)).matched), acceptor_par, lasers, im_par, ...
    %         w_events_D, starting_states_D, ending_states_D, w_events_A, starting_states_A, ending_states_A, ...
    %         state_link_D, state_link_A, fluo_states_D, fluo_states_A, R0_link_D, R0_link_A,during_frametime);
...
if ~isempty(unpaired_indices_D)
    parfor (k=1:numel(unpaired_indices_D), im_par.parforArg)
        if addtime>0
            during_frametime=0;
            unpaired_donor_sm(k)=get_state_evolution(unpaired_donor_sm(k),lasers, donor_par, im_par, w_events_D, starting_states_D, ending_states_D, during_frametime);
        end
        during_frametime=1;
        unpaired_donor_sm(k)=get_state_evolution(unpaired_donor_sm(k),lasers, donor_par, im_par, w_events_D, starting_states_D, ending_states_D,during_frametime);
    end
...
if ~isempty(unpaired_indices_A)
    parfor (k=1:numel(unpaired_indices_A), im_par.parforArg) 
        if addtime>0
            during_frametime=0;
            unpaired_acceptor_sm(k)=get_state_evolution(unpaired_acceptor_sm(k),lasers, acceptor_par, im_par, w_events_A, starting_states_A, ending_states_A, during_frametime);
        end
        during_frametime=1;
        unpaired_acceptor_sm(k)=get_state_evolution(unpaired_acceptor_sm(k),lasers, acceptor_par, im_par, w_events_A, starting_states_A, ending_states_A, during_frametime);
    end
...
donor.sm(paired_indices_D)=donor_sm;
if ~isempty(unpaired_indices_D)
    donor.sm(unpaired_indices_D)=unpaired_donor_sm;
end

acceptor.sm(paired_indices_A)=acceptor_sm;
if ~isempty(unpaired_indices_A)
    acceptor.sm(unpaired_indices_A)=unpaired_acceptor_sm;
end
