function [subTrace,sampling_rate]=get_fret_subtrace(sm,sm_par,N,sampling_rate,start_time,end_time, im_par, w_events, state_link)
...
trans_p_k=sm_par.trans_k(sm_par.state_ids==sm.state,:)/sampling_rate;
...
trans_p_q = 1-bsxfun(@power,1-sm_par.trans_q(sm_par.state_ids==sm.state,:),N'); % transition matrix for photo activated processes (each row should be powered by a corresponding N)
...
subTrace=[start_time;sm.state]; % initialize subtrace
...
if ~isempty(Trace)
    ...
    %process the 1st event that involves current sm.state or states in
    %equilibrium with sm.state
    S=state_link(sm.state).starting_states(state_link(sm.state).index==Trace(2,1)); % What is the starting state for this transition
    F=state_link(sm.state).ending_states(state_link(sm.state).index==Trace(2,1)); % What is the ending state for this transition
    ...
    subTrace = horzcat(subTrace,[end_time;sm.state]); % simply close up trace
end
...
if plot_traces==1
    ...
    set(p,'LineWidth',1.5);
    set(p,'Color','blue');
    ylim([0 max(state_link(sm.state).ending_states)]);
    ...
    disp(['New photophysical state:' , num2str(sm.state)]);
    input('Ok ? ','s');
end
