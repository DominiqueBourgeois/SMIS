function sm=get_fluorescence_in_fret_mode(sm, sm_par, im_par)
...
for i=1:sm_par.n_fluorescent_states
    if sm_par.fluorescent_states(i)>=sm_par.initial_fluo_state % Only treat accessible fluorescent states
        ...
        if sm.matched>0 % The molecule is in interaction with a FRET partner           
            sm.n_abs(sm_par.fluorescent_states(i))=n_abs-sm.given_fret_photons(i)+sm.received_fret_photons(i);
        else
            sm.n_abs(sm_par.fluorescent_states(i))=n_abs;
        end
               
        % Get # of emitted photons during that time; use poissonian statistics
        if sm.n_abs(sm_par.fluorescent_states(i))>0; n_em=poissrnd(sm_par.quantum_yield(i)*sm.n_abs(sm_par.fluorescent_states(i))); else n_em=0; end
        sm.n_em(i)=n_em;
