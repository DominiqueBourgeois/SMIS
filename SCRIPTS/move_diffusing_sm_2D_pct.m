function  sms = move_diffusing_sm_2D_pct(n_fluorophores, sms, sm_par, im_par, display_par)
...
for i=1:n_fluorophores
    ...
    if ~sm_par(i).is_acceptor
        ...
        for j=1:N
            ...
            if ~isempty(w_D_act)
                sms(i).sm_cell(:,w_D_act) = update_diffusing_sm_2D_pct(sms(i).sm_cell(:,w_D_act), im_par, sm_par(i), j);
            end
        end
        ...
        if im_par.fluorophore_pairing_on==1
            ...
            if ~isempty(w_unmatched)
                ...
                for j=1:N
                    ...
                    if ~isempty(w_D_act)
                        sms(td_id).sm_cell(:,w_D_act) = update_diffusing_sm_2D_pct(sms(td_id).sm_cell(:,w_D_act), im_par, sm_par(td_id), j);
                    end
                end
            end
