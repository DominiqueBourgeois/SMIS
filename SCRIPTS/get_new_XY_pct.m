function [x_h_d, y_h_d,sm] = get_new_XY_pct(x_h, y_h, sm, D_ras, dt, S, sm_par, sm_pattern_indices, im_par)
...
if force_change==1
    ...
    %there
    if n_sp~=c_sp
        P = move_molecule_to_new_pattern([x_h,y_h],w_patterns(sm_par.n_sp_id==n_sp).w,im_par);
        x_h_d=P(1)+rand;
        y_h_d=P(2)+rand;
        sm{diff_state_idx}=n_ds; % update diffusion state
        sm{c_sp_idx}=n_sp; % update current pattern
        if sm_par.V(n_ds)>0 % % update velocity
            v_h = get_initial_directions(x_h_d, y_h_d, 0, n_sp, n_ds, im_par, sm_par, w_patterns);
            sm{v_x_idx}=v_h(1);
            sm{v_y_idx}=v_h(2);
        end
        return % Stop here !
...
% Check whether moving molecule passes through undesired subpatterns
if sm_par.Hop_Diffusion==1 % Store starting positions
    x_h_s=x_h;
    y_h_s=y_h;
end
crossing_found=1; % Start by running the loop at least once anyway

while crossing_found
    %Handle directed motion if necessary
    if sm_par.V(c_ds)>0
        [p_h_out,v_h_out,p_h_out_np]=update_directed_motion_2D([x_h,y_h],[sm{v_x_idx}, sm{v_y_idx}], dt, c_ds, n_ds, sm_par, sm_pattern_indices, im_par);
    ...
    if D_ras(c_ds)==0
        ...
        if n_ds~=c_ds && sm_par.D_confined(n_ds)==sm_par.D_confined(c_ds) % Case where there is
            ...
            if sm_par.V(c_ds)>0 % eventually update current velocity Changed 17/01/21
                v_h=[sm{v_x_idx}, sm{v_y_idx}]; % This is the current velocity vector
    ...
    if sm_par.Hop_Diffusion==1
        crossing_found=get_potential_crossing_XY([x_h_s,y_h_s],[x_h_d,y_h_d],c_sp,n_sp,sm_par.n_sp_id, w_patterns, S);
        if crossing_found==1
            Test=rand;
