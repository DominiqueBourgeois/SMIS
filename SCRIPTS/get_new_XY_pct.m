...
if sm_par.V(c_ds)>0
    ...
    if isempty(p_h_out_np) % Normal case, no change of pattern possible
        ...
        %             sm{diff_state_idx}=n_ds;
        %             sm{v_x_idx}=v_h_out(1)/norm(v_h_out)*1e+3*sm_par.V(n_ds)/im_par.raster*im_par.binning;
        %             sm{v_y_idx}=v_h_out(2)/norm(v_h_out)*1e+3*sm_par.V(n_ds)/im_par.raster*im_par.binning;
        %         end
