function N=get_number_of_absorbed_photons_pct(sm, lasers, sm_par, im_par, sampling_rate)
...
if im_par.simul_3D==1
    [x,y,z]=get_coordinates_on_detector_pct(sm{x_idx},sm{y_idx},sm{z_idx}, im_par); % x,y,z in raster
    ...
    [x,y,~]=get_coordinates_on_detector_pct(sm{x_idx},sm{y_idx},[], im_par); % x,y,z in raster
    ...
    if (im_par.during_frametime==0 && lasers(i).on_during_addtime==1) || (im_par.during_frametime==1 && lasers(i).on_during_frametime==1)
        if lasers(i).sequence(im_par.current_frame) > 0 && ~isempty(lasers(i).beam_profile)
            ...
            for j=1:sm_par.n_fluorescent_states
                ...
                if sm{state_idx}<sm_par.converted_state || (sm{state_idx}>=sm_par.converted_state && fluo_state_id >= sm_par.converted_state)
            ...
            for j=1:sm_par.n_photoactive_dark_states
                ...
                % Only scan for possible states if molecule already photoconverted
                if sm{state_idx}<sm_par.converted_state || (sm{state_idx}>=sm_par.converted_state && dark_state_id >= sm_par.converted_state)
                    %The cross section is given by
            ...
            if sm_par.anisotropy==1 % take anisotropy into account
                theta=sm{theta_idx}; phi=sm{phi_idx};
