...
%	D.Bourgeois, February 2024, corrected bug concerning the contribution of different fluorescent states with their associated pointspread function and recorded number of photons to the detected images.
...
n_phot_det_ch1=zeros(1,sm_par.n_fluorescent_states);
n_phot_det_ch2=zeros(1,sm_par.n_fluorescent_states);
...
for i=1:sm_par.n_fluorescent_states
    ...
    if channel==1 && sm{n_phot_ch1_idx}(i)>0
        ...
        n_phot_det_ch1(i)=get_emitted_photons(sm{n_phot_ch1_idx}(i), sm{theta_idx}, im_par, sm_par);
    elseif channel==2 && sm{n_phot_ch2_idx}(i)>0 %same for channel 2
        %get the number of photons actually reaching the detector
        n_phot_det_ch2(i)=get_emitted_photons(sm{n_phot_ch2_idx}(i), sm{theta_idx}, im_par, sm_par);
    end
...
if channel==1 && any(n_phot_det_ch1)
    %update sm{n_phot_det_ch1_idx}
    sm{n_phot_det_ch1_idx}=[sm{n_phot_det_ch1_idx},sum(n_phot_det_ch1)];
    ...
    %get the number of photons actually reaching the detector
    for i=1:sm_par.n_fluorescent_states
        if n_phot_det_ch1(i)>0
            if im_par.simul_3D==0
                if isempty(peak)
                    peak=get_2Dpeak(x,y,n_phot_det_ch1(i),sm_par,im_par,i,channel);
                else
                    peak=peak+get_2Dpeak(x,y,n_phot_det_ch1(i),sm_par,im_par,i,channel);
                end
            else
                if isempty(peak)
                    peak=get_3Dpeak(x,y,z, n_phot_det_ch1(i), sm_par,im_par,i,channel);
                else
                    peak=peak+get_3Dpeak(x,y,z, n_phot_det_ch1(i), sm_par,im_par,i,channel);
                end
            end
        end
    end
...
elseif channel==2 && any(n_phot_det_ch2) %same for channel 2
    %update sm{n_phot_det_ch2_idx}
    sm{n_phot_det_ch2_idx}=[sm{n_phot_det_ch2_idx},sum(n_phot_det_ch2)];
    ...
    %get the number of photons actually reaching the detector
    for i=1:sm_par.n_fluorescent_states
        if n_phot_det_ch2(i)>0
            if im_par.simul_3D==0
                if isempty(peak)
                    peak=get_2Dpeak(x,y,n_phot_det_ch2(i),sm_par,im_par,i,channel);
                else
                    peak=peak+get_2Dpeak(x,y,n_phot_det_ch2(i),sm_par,im_par,i,channel);
                end
            else
                if isempty(peak)
                    peak=get_3Dpeak(x,y,z,n_phot_det_ch2(i),sm_par,im_par,i,channel);
                else
                    peak=peak+get_3Dpeak(x,y,z,n_phot_det_ch2(i),sm_par,im_par,i,channel);
                end
            end
        end
    end
...
sm{tot_phot_det_ch1_idx}=sm{tot_phot_det_ch1_idx}+sum(n_phot_det_ch1);
sm{tot_phot_det_ch2_idx}=sm{tot_phot_det_ch2_idx}+sum(n_phot_det_ch2);
